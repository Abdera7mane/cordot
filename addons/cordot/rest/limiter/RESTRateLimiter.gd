...
var Packed: HTTPConnectionPool

func _init(use_Packed: bool = false) -> void:
	if use_Packed:
		Packed = HTTPConnectionPool.new(5, 50)
...
func request_async(request: RestRequest) -> HTTPResponse:
	return await Packed.request_async(
		request.url,
		request.headers,
		true,
		request.method,
		request.body
	) if Packed else await request.send_async()
...
func get_bucket(request: RestRequest) -> Array:
	...
	var parameters: PackedStringArray = path.split("/", false)
...
func queue_request(request: RestRequest) -> HTTPResponse:
	...
	if global:
		Awaiter.wait(global_reset - Time.get_ticks_msec())
	...
	Awaiter.wait(total_wait_time)

	var response: HTTPResponse
	while true:
		var _start: int = Time.get_ticks_msec()
		response = await request_async(request)
		last_latency_ms = Time.get_ticks_msec() - _start

		if response.code == HTTPClient.RESPONSE_TOO_MANY_REQUESTS:
			push_error("Discord REST: Rate Limited, url: %s" % request.url)

			var json: = JSON.new()
			if json.parse(response.body.get_string_from_utf8()) != OK:
				push_error("JSON Error: %s at %s" % [json.get_error_message(), json.get_error_line()])
				print_rich("[]" % "JSON Error: %s at %s" % [json.get_error_message(), json.get_error_line()])
			var rate_limit_data: Dictionary = json.get_data()
			...
			if global:
				...
				global_reset = _retry_after + Time.get_ticks_msec()
			Awaiter.wait(_retry_after)
			global = false
			retry_after = 0
			global_reset = 0
			continue
	...
	if not bucket_hash.is_empty():
		routes[route] = bucket_hash
		if route_bucket.key.is_empty():
