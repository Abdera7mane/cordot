...
class MarkdownGenerator:
	...
	func create_document(class_reference: Dictionary) -> MarkdownDocument:
		...
		var parents: PackedStringArray = _get_class_parents(class_reference)
		var inheriters: Array = _get_class_inheriters(class_reference)
		# sort() method is only available in 3.5+ for Packed Arrays
		inheriters.sort()

		var formatted_parents := PackedStringArray()
		var formatted_inheriters := PackedStringArray()
		...
		if not skip_properties_description:
			...
			for property in properties:
				...
				var i: int = 1
				if not (setter.is_empty() or setter.begins_with("_")):
					table.put("*Setter*", i, 0)
					table.put("%s(value)" % MarkdownDocument.escape_md(setter), i, 1)
					i += 1
				if not (getter.is_empty() or getter.begins_with("_")):
	...
	func _get_class_parents(class_reference: Dictionary) -> PackedStringArray:
		var parents: PackedStringArray = class_reference["extends_class"]
		if parents.size() == 0:
	...
	func _get_class_inheriters(class_reference: Dictionary) -> PackedStringArray:
		var name: String = class_reference["name"]
		var inheriters := PackedStringArray()
		for _class in _classes.values():
			var extends_class: PackedStringArray = _class["extends_class"]
	...
	func _get_method_signature(method_reference: Dictionary, with_link: bool = true, is_static: bool = false) -> String:
		...
		for parameter in method_reference["arguments"]:
			...
			if default_rest and default_value.is_empty():
	...
	func _get_signal_signature(signal_reference: Dictionary) -> String:
		...
		var parameters: PackedStringArray = signal_reference["arguments"]
